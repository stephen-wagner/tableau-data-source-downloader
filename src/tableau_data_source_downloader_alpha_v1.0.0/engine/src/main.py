...
factory = PluginFactory("tableau_data_source_downloader_alpha_v1.0.0")
...
def init(input_mgr, user_data, logger):
    ...
    if user_data.data_source_name == None:
        logger.display_error_msg("Enter data source name")
        return False

    if user_data.download_location == None:
        logger.display_error_msg("Enter location to save data source")
        return False
